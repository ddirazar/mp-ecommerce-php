...
switch($_SERVER['REQUEST_METHOD'])
{
    ...
    case 'POST': $parametros = $_PO7ST; break;
}

$resultado = "Su pago ha sigo aprobado exitosamente";

$rta_pago = CallApi(MP_API_URL,
               "GET",
               "/v1/payments/".$parametros["collection_id"]."?access_token=".MP_ACCESS_TOKEN,
               null);
//
// TODO: Manejo de errores
// 
$rta_medios_pago = CallApi(MP_API_URL,
               "GET",
               "/v1/payment_methods?access_token=".MP_ACCESS_TOKEN,
               null);
...
$descripcion = $rta_pago["response"]["description"];
$monto = $rta_pago["response"]["transaction_details"]["total_paid_amount"];
$order_id=$rta_pago["response"]["order"]["id"];

foreach ($rta_medios_pago["response"] as $key => $value) {
    # code...
    if( $value["id"]===$rta_pago["response"]["payment_method_id"]){
        $medio_pago = $value["name"]." ";
    }
}
                                        ...
                                        <div class="as-producttile-title">
                                            <h3 class="as-producttile-name">
                                                <p class="as-producttile-tilelink">
                                                    <span data-ase-truncate="2"> <?php echo " "  ?></span>
                                                </p>

                                            </h3>
                                        </div>
                                        <h2 >
                                            Se ha completado el pago del artículo :<?php echo $descripcion ?>
                                        </h2>
                                        <h3>
                                            Referencia :<?php echo isset($parametros["external_reference"]) ? $parametros["external_reference"]: " " ?>
                                        </h3>
                                        <h2>
                                            Monto de la operación :<?php echo $monto ?>
                                        </h2>
                                        <h3 >
                                            Orden de Pedido :<?php echo $order_id ?>
                                        </h3>
                                        <button type="button" class="mercadopago-button" 
                                        onclick="location.href='index.php'">    Volver                    
                                        </button>
                                    </div>
